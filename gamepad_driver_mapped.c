...
static void gamepad_read_int_callback(struct urb *urb)
{
	...
	input_report_key(dev, BTN_MIDDLE,  data[3] & BUTTON_X);
	input_report_key(dev, BTN_SIDE,  data[3] & BUTTON_Y);
	...
	/* right stick */
	input_report_rel(dev, REL_HWHEEL, (__s16) le16_to_cpup((__le16 *)(data + 10)));
	// input_report_rel(dev, REL_WHEEL, ~(__s16) le16_to_cpup((__le16 *)(data + 12)));
	input_report_rel(dev, ABS_WHEEL, ~(__s16) le16_to_cpup((__le16 *)(data + 12))/2048);
